...
public class UsaToy_rvAdapter extends RecyclerView.Adapter<UsaToy_rvAdapter.ToyViewHolder> {

    ArrayList<Toy_org> toy_org = new ArrayList<>();
    private Context toy_context;

    public UsaToy_rvAdapter(Context toy_context){
        this.toy_context = toy_context;
    }

    public UsaToy_rvAdapter() {

    }

    @NonNull
    @Override
    public UsaToy_rvAdapter.ToyViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.food_org_list, parent, false);
        UsaToy_rvAdapter.ToyViewHolder holder = new UsaToy_rvAdapter.ToyViewHolder(v);
        return holder;
    }

    @Override
    public void onBindViewHolder(@NonNull UsaToy_rvAdapter.ToyViewHolder holder, @SuppressLint("RecyclerView") int position) {
