...
public class IndiaCovid_rvAdapter extends RecyclerView.Adapter<IndiaCovid_rvAdapter.CovidViewHolder> {

    ArrayList<Covid_org> covid_org = new ArrayList<>();
    private Context covid_context;

    public IndiaCovid_rvAdapter (Context covid_context){
        this.covid_context = covid_context;
    }

    public IndiaCovid_rvAdapter() {

    }

    @NonNull
    @Override
    public IndiaCovid_rvAdapter.CovidViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.food_org_list, parent, false);
        IndiaCovid_rvAdapter.CovidViewHolder holder = new IndiaCovid_rvAdapter.CovidViewHolder(v);
        return holder;
    }

    @Override
    public void onBindViewHolder(@NonNull IndiaCovid_rvAdapter.CovidViewHolder holder, @SuppressLint("RecyclerView") int position) {
