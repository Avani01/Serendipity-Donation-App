...
public class UsaChild_rvAdapter extends RecyclerView.Adapter<UsaChild_rvAdapter.ChildViewHolder> {

    ArrayList<Child_org> child_org = new ArrayList<>();
    private Context child_context;

    public UsaChild_rvAdapter (Context child_context){
        this.child_context = child_context;
    }

    public UsaChild_rvAdapter() {

    }

    @NonNull
    @Override
    public UsaChild_rvAdapter.ChildViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.food_org_list, parent, false);
        UsaChild_rvAdapter.ChildViewHolder holder = new UsaChild_rvAdapter.ChildViewHolder(v);
        return holder;
    }

    @Override
    public void onBindViewHolder(@NonNull UsaChild_rvAdapter.ChildViewHolder holder, @SuppressLint("RecyclerView") int position) {
