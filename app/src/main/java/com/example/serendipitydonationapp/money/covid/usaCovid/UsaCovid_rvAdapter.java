...
public class UsaCovid_rvAdapter extends RecyclerView.Adapter<UsaCovid_rvAdapter.CovidViewHolder> {

    ArrayList<Covid_org> covid_org = new ArrayList<>();
    private Context covid_context;

    public UsaCovid_rvAdapter (Context covid_context){
        this.covid_context = covid_context;
    }

    public UsaCovid_rvAdapter() {

    }

    @NonNull
    @Override
    public UsaCovid_rvAdapter.CovidViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.food_org_list, parent, false);
        UsaCovid_rvAdapter.CovidViewHolder holder = new UsaCovid_rvAdapter.CovidViewHolder(v);
        return holder;
    }

    @Override
    public void onBindViewHolder(@NonNull UsaCovid_rvAdapter.CovidViewHolder holder, @SuppressLint("RecyclerView") int position) {
