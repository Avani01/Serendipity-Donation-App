package com.example.serendipitydonationapp.money.pod.uaePod;
...
public class Pod_rvAdapter extends RecyclerView.Adapter<Pod_rvAdapter.PodViewHolder> {
    ArrayList<Pod_org> pod_org = new ArrayList<>();
    private Context pod_context;

    public Pod_rvAdapter (Context pod_context){
        this.pod_context = pod_context;
    }

    public Pod_rvAdapter() {

    }

    @NonNull
    @Override
    public Pod_rvAdapter.PodViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.food_org_list, parent, false);
        Pod_rvAdapter.PodViewHolder holder = new Pod_rvAdapter.PodViewHolder(v);
        return holder;
    }

    @Override
    public void onBindViewHolder(@NonNull Pod_rvAdapter.PodViewHolder holder, @SuppressLint("RecyclerView") int position) {
